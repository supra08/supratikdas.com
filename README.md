...
This site includes a complete newsletter subscription system with the following components:

### Main Site (Static)
- Subscribe form on homepage
- Unsubscribe page with token-based verification
- Supabase integration for subscriber management

### Admin Panel (Separate Project)
- Modern admin interface for sending newsletters
...
- Handles both newsletter and test email sending
- Located in `/supabase/functions/send-newsletter/`

### Setup Instructions

1. **Environment Variables** (add to `.env`):
   ```env
   SUPABASE_URL=https://qplnzqliygxjiydsmgui.supabase.co
   SUPABASE_KEY=your-supabase-anon-key
   MAILGUN_API_KEY=your-mailgun-api-key
   MAILGUN_DOMAIN=your-mailgun-domain
   MAILGUN_SENDER=your-sender-email
   SITE_URL=https://supratikdas.com
   ```

2. **Deploy Edge Function**:
   ```bash
   cd supabase/functions/send-newsletter
...
See individual README files in `/admin-panel/` and `/supabase/functions/send-newsletter/` for detailed setup instructions.
